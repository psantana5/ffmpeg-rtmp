...
class EnergyEfficiencyScorer:
    ...
    def _compute_pixels_per_joule(self, scenario: Dict) -> Optional[float]:
        ...
        if total_energy_joules <= 0:
            logger.debug(
                f"Scenario '{scenario.get('name')}': Invalid energy value {total_energy_joules}"
            )
            return None
    ...
    def compute_cost_adjusted_score(self, scenario: Dict, cost_per_kwh: float) -> Optional[float]:
        """
