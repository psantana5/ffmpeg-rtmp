...
class TestScenario:
    ...
    def __init__(self, name: str, bitrate: str, resolution: str = "1280x720", 
                 fps: int = 30, duration: int = 300):
        self.name = name
...
class TestRunner:
    ...
    def run_scenario(self, scenario: TestScenario, stabilization_time: int = 30) -> Dict:
        """Run a single test scenario"""
        logger.info(f"Starting scenario: {scenario.name}")
        logger.info(f"  Bitrate: {scenario.bitrate}, Resolution: {scenario.resolution}, FPS: {scenario.fps}")
        
        # Build ffmpeg command
        cmd = [
            'ffmpeg', '-re',
            '-f', 'lavfi', '-i', f'testsrc=size={scenario.resolution}:rate={scenario.fps}',
            '-f', 'lavfi', '-i', 'sine=frequency=1000:sample_rate=48000',
            '-c:v', 'libx264', '-preset', 'veryfast', '-tune', 'zerolatency',
            '-b:v', scenario.bitrate, '-maxrate', scenario.bitrate,
            '-bufsize', f"{int(scenario.bitrate.replace('k', '')) * 2}k",
            '-pix_fmt', 'yuv420p', '-g', str(scenario.fps * 2),
            '-c:a', 'aac', '-b:a', '128k', '-ar', '48000',
            '-f', 'flv', f'rtmp://localhost:1935/live/{scenario.name.replace(" ", "_")}'
        ]
    ...
    def run_multiple_streams(self, count: int, bitrate: str, duration: int = 300,
                            stabilization_time: int = 30) -> Dict:
        ...
        try:
            ...
            for i in range(count):
                cmd = [
                    'ffmpeg', '-re',
                    '-f', 'lavfi', '-i', f'testsrc=size=1280x720:rate=30',
                    '-f', 'lavfi', '-i', 'sine=frequency=1000:sample_rate=48000',
                    '-c:v', 'libx264', '-preset', 'veryfast', '-tune', 'zerolatency',
                    ...
                    '-c:a', 'aac', '-b:a', '128k', '-ar', '48000',
                    '-f', 'flv', f'rtmp://localhost:1935/live/stream{i}'
                ]
