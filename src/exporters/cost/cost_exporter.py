...
class CostMetricsExporter:
    ...
    def enrich_scenario_with_prometheus(self, scenario: Dict) -> Dict:
        ...
        try:
            ...
            if cpu_values:
                ...
                logger.debug(
                    f"Scenario '{scenario_name}': Enriched with {len(cpu_values)} CPU measurements"
                )
            else:
                logger.warning(
                    f"Scenario '{scenario_name}': No CPU usage data returned from Prometheus"
                )
            ...
            if power_values:
                ...
                logger.warning(
                    f"Scenario '{scenario_name}': No power data returned from Prometheus"
                )

        except Exception as e:
            logger.error(f"Failed to enrich scenario '{scenario_name}': {e}")
    ...
    def generate_prometheus_metrics(self) -> str:
        ...
        for scenario in scenarios:
            ...
            if not bitrate:
                logger.debug(f"Skipping scenario '{scenario_name}': missing bitrate label")
                continue
            ...
            has_load_aware_data = 'cpu_usage_cores' in scenario and 'power_watts' in scenario
            ...
            if has_load_aware_data:
                ...
                if cost_per_pixel is not None:
                    output.append(f"cost_per_pixel{{{labels}}} {cost_per_pixel:.12f}")
                    logger.debug(f"Set cost_per_pixel{{{safe_name}}}={cost_per_pixel:.12f}")
                    metrics_emitted += 1
                if cost_per_watch_hour is not None:
                    output.append(f"cost_per_watch_hour{{{labels}}} {cost_per_watch_hour:.8f}")
                    logger.debug(
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description='Cost Metrics Prometheus Exporter')
    ...
    parser.add_argument(
        '--energy-cost', type=float, default=0.0, help='Energy cost per kWh (default: 0.0)'
    )
    parser.add_argument(
        '--cpu-cost', type=float, default=0.0, help='CPU/instance cost per hour (default: 0.0)'
    )
    parser.add_argument(
        '--gpu-cost', type=float, default=0.0, help='GPU cost per hour (default: 0.0)'
    )
