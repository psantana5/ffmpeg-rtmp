...
class ResultsExporter:
    ...
    def _compute_scenario_stats(self, start: float, end: float, scenario: dict) -> dict:
        ...
        if (
            isinstance(fps_val, (int, float))
            and energy_j
            and duration > 0
            and (fps_val * duration) > 0
        ):
        ...
        prediction_confidence_low = (
            max(0.0, mean_power - (2.0 * power_stdev)) if mean_power > 0 else 0.0
        )
    ...
    def build_metrics(self) -> str:
        ...
        output.append(
            "# HELP results_scenario_mean_power_watts Mean CPU package power (W) during scenario"
        )
        output.append("# TYPE results_scenario_mean_power_watts gauge")
        output.append(
            "# HELP results_scenario_measured_power_watts Measured CPU package power (W) from RAPL"
        )
        ...
        output.append(
            "# HELP results_scenario_net_container_cpu_percent Container CPU percent above baseline"
        )
        ...
        output.append(
            "# HELP results_scenario_power_pct_increase Power percent increase vs baseline"
        )
        output.append("# TYPE results_scenario_power_pct_increase gauge")
        output.append(
            "# HELP results_scenario_efficiency_score Energy efficiency score (pixels/J or Mbps/W)"
        )
        output.append("# TYPE results_scenario_efficiency_score gauge")
        output.append(
            "# HELP results_scenario_total_pixels Total pixels delivered across all outputs"
        )
        ...
        output.append(
            "# HELP results_scenario_power_stdev Standard deviation of power measurements (W)"
        )
        ...
        if baseline and baseline.get("start_time") and baseline.get("end_time"):
            baseline_stats = self._compute_scenario_stats(
                baseline["start_time"], baseline["end_time"], baseline
            )
        ...
        for scenario in scenarios:
            ...
            scenario_copy['container_usage'] = {'cpu_percent': stats['container_cpu_percent']}
        ...
        if PREDICTOR_AVAILABLE:
            valid_scenarios = [
                s for s, _ in scenarios_with_stats if s.get('power', {}).get('mean_watts')
            ]
            should_train = len(valid_scenarios) >= 3 and (
        ...
        for scenario_copy, stats in scenarios_with_stats:
            ...
            scenario = next(
                (s for s in scenarios if s.get('name') == scenario_copy.get('name')), scenario_copy
            )
            ...
            if stats["energy_wh_per_min"] is not None:
                output.append(
                    f"results_scenario_energy_wh_per_min{lbl} {stats['energy_wh_per_min']:.6f}"
                )
            if stats["energy_wh_per_mbps"] is not None:
                output.append(
                    f"results_scenario_energy_wh_per_mbps{lbl} {stats['energy_wh_per_mbps']:.6f}"
                )
            if stats["energy_mj_per_frame"] is not None:
                output.append(
                    f"results_scenario_energy_mj_per_frame{lbl} {stats['energy_mj_per_frame']:.4f}"
                )
            ...
            # Export power stdev and confidence interval from measured data
            if 'power_stdev_w' in stats:
