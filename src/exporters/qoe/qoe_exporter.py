...
class QoEMetricsExporter:
    ...
    def generate_prometheus_metrics(self) -> str:
        ...
        output.append(
            "# HELP qoe_efficiency_score QoE efficiency score (quality-weighted pixels/joule)"
        )
        output.append("# TYPE qoe_efficiency_score gauge")
        output.append("# HELP qoe_computation_duration_seconds Time to compute quality metrics")
        ...
        for scenario in scenarios:
            ...
            if qoe_metrics['vmaf_score'] is not None:
                output.append(f"qoe_vmaf_score{{{labels}}} {qoe_metrics['vmaf_score']:.2f}")

            # Export PSNR
            if qoe_metrics['psnr_score'] is not None:
                output.append(f"qoe_psnr_score{{{labels}}} {qoe_metrics['psnr_score']:.2f}")

            # Export quality per watt
            if qoe_metrics['quality_per_watt'] is not None:
                output.append(
                    f"qoe_quality_per_watt{{{labels}}} {qoe_metrics['quality_per_watt']:.6f}"
                )

            # Export QoE efficiency score
            if qoe_metrics['qoe_efficiency_score'] is not None:
                output.append(
                    f"qoe_efficiency_score{{{labels}}} {qoe_metrics['qoe_efficiency_score']:.4e}"
                )
