...
class PowerPredictor:
    ...
    def fit(self, scenarios: List[Dict]) -> bool:
        ...
        if unique_streams >= 6:
            ...
            logger.info(
                f"Using polynomial regression (degree=2) with {unique_streams} unique stream counts"
            )
...
class MultivariatePredictor:
    ...
    def fit(
        ...
        if len(features_list) < 2:
            logger.warning(
                f"Insufficient data for multivariate predictor: {len(features_list)} samples"
            )
            return False
            ...
            try:
                ...
                if len(X) >= self.cv_folds:
                    cv_scores = cross_val_score(
                        pipeline, X, y, cv=min(self.cv_folds, len(X)), scoring='r2'
                    )
        ...
        self.best_model_name = max(
            self.model_scores.keys(), key=lambda k: self.model_scores[k]['r2']
        )
    ...
    def predict(
        self, features: Dict, return_confidence: bool = True, model_name: Optional[str] = None
    ) -> Dict:
        """
    ...
    def _bootstrap_confidence_interval(self, x: np.ndarray, model_name: str) -> Tuple[float, float]:
        """
    ...
    def predict_batch(
        self, features_list: List[Dict], return_confidence: bool = False
    ) -> List[Dict]:
        """
        ...
        for i, pred in enumerate(predictions):
            result = {'mean': float(max(0.0, pred)), 'model': self.best_model_name}

            if return_confidence:
                ci_low, ci_high = self._bootstrap_confidence_interval(X[i], self.best_model_name)
                result['ci_low'] = float(max(0.0, ci_low))
                result['ci_high'] = float(max(0.0, ci_high))
                result['ci_width'] = result['ci_high'] - result['ci_low']
