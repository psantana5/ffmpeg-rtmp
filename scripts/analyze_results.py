...
class ResultsAnalyzer:
    ...
    def generate_report(self) -> List[Dict]:
        ...
        power_results = [
            r for r in results if 'power' in r and r['power'].get('mean_watts') is not None
        ]
    ...
    def print_summary(self, results: List[Dict]):
        ...
        if scored_results:
            ...
            print("─" * 100)
            header = (
                f"{'Rank':<6} {'Scenario':<35} {'Efficiency':<18} {'Power':<12} {'Bitrate':<12}"
            )
            print(header)
            print("─" * 100)
        ...
        if results and results[0].get('_by_ladder'):
            ...
            if len(valid_ladders) > 1:
                ...
                print("─" * 100)
                print(
                    "\nScenarios grouped by output ladder (identical resolution/fps combinations)"
                )
    ...
    def print_multivariate_predictions(
        ...
        typical_scenario = next(
            (r for r in results if 'baseline' not in r['name'].lower()), results[0]
        )
    ...
    try:
        ...
        if args.multivariate:
            ...
            if success:
                ...
                logger.warning(
                    "Multivariate predictor training failed, falling back to simple predictor"
                )
