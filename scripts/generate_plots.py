...
def plot_power_vs_bitrate(df: pd.DataFrame, output_dir: Path):
    ...
    # Create scatter plot
    scatter = ax.scatter(
        test_df['bitrate_mbps'],
        test_df['mean_power_w'],
        c=test_df['mean_power_w'],
        cmap='viridis',
        s=100,
        alpha=0.7,
        edgecolors='black',
...
def plot_power_by_streams(df: pd.DataFrame, output_dir: Path):
    ...
    if not stream_tests.empty:
        ...
        # Create bar plot
        bars = ax.bar(
            stream_tests['num_streams'],
            stream_tests['mean_power_w'],
            color='#1f77b4',
            alpha=0.8,
            edgecolor='black',
            linewidth=1,
        ...
        if not baseline_df.empty and not stream_tests.empty:
            ...
            per_stream = [
                (p - baseline_power) / n
                for p, n in zip(stream_tests['mean_power_w'], stream_tests['num_streams'])
            ]
...
def plot_efficiency_comparison(df: pd.DataFrame, output_dir: Path):
    ...
    if not test_df.empty:
        ...
        test_df['wh_per_mbps'] = test_df.apply(
            lambda row: (
                row['energy_wh'] / row['bitrate_mbps'] if row['bitrate_mbps'] > 0 else np.nan
            ),
            axis=1,
        )
        ...
        ax2.scatter(
            valid_data['bitrate_mbps'],
            valid_data['mean_power_w'],
            c=valid_data['wh_per_mbps'],
            cmap='plasma',
            s=100,
            alpha=0.7,
        )
        ...
        bars4 = ax4.bar(
            metrics, values, color=['#1f77b4', '#2ca02c', '#ff7f0e'], alpha=0.8, edgecolor='black'
        )
...
def create_summary_dashboard(df: pd.DataFrame, output_dir: Path):
    ...
    ax3.scatter(
        df['duration'], df['mean_power_w'], alpha=0.7, c=df['mean_power_w'], cmap='viridis', s=100
    )
    ...
    if not energy_data.empty:
        ax4.bar(
            range(len(energy_data)), energy_data['total_energy_wh'], alpha=0.8, edgecolor='black'
        )
...
def main():
    ...
    parser.add_argument(
        '--input', '-i', type=str, required=True, help='Path to test results JSON file'
    )
    parser.add_argument('--csv', '-c', type=str, help='Path to analysis CSV file (optional)')
    parser.add_argument(
        '--output', '-o', type=str, default='plots', help='Output directory for plots'
    )
