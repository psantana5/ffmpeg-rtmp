...
class PrometheusClient:
    ...
    def query_range(self, query: str, start: float, end: float, step: str = '5s') -> Optional[Dict]:
        ...
        params = {'query': query, 'start': float(start), 'end': float(end), 'step': step}
...
class ModelRetrainer:
    ...
    def _save_power_predictor(self, predictor: PowerPredictor, path: Path, info: Dict):
    ...
    def generate_metadata(self) -> Dict:
        ...
        metadata = {
            ...
            'models': {
                'power_predictor': {'path': 'power_predictor_latest.pkl', 'type': 'PowerPredictor'},
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description='Retrain ML models from test results')
    parser.add_argument(
        '--results-dir',
        type=Path,
    ...
    parser.add_argument(
        '--hardware-id', type=str, help='Hardware identifier (auto-detected if not provided)'
    )
    parser.add_argument(
        '--prometheus-url',
        type=str,
