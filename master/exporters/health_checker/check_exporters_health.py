...
class ExporterHealthChecker:
    ...
    def check_exporter(self, config: ExporterConfig) -> HealthCheckResult:
        ...
        has_expected, missing = self.check_expected_metrics(metrics_text, config.expected_metrics)
    ...
    def generate_prometheus_metrics(self) -> str:
        ...
        output.append(
            "# HELP exporter_health_status Health status of exporter (1=healthy, 0=unhealthy)"
        )
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description='Check health of Prometheus exporters')
    parser.add_argument(
        '--interval', type=int, default=0, help='Check interval in seconds (0 for single check)'
    )
    parser.add_argument(
        '--port', type=int, default=0, help='Port to expose metrics (0 for no server)'
    )
    parser.add_argument(
    ...
    parser.add_argument(
        '--no-unicode', action='store_true', help='Disable Unicode characters in output'
    )
    ...
    checker = ExporterHealthChecker(
        EXPORTERS, timeout=args.timeout, use_unicode=not args.no_unicode
    )
